...
def _render_day_cell(day_iso: str, day_data: Dict[str, Any], is_today: bool = False) -> str:
    ...
    def render_column(title: str, categories: list, meal_type: str) -> str:
        col_parts = [f"<div class='text-xs md:text-base font-semibold text-gray-700 mb-2'>{title}</div>"]
        has_content = False
        ...
        for cat in categories:
            ...
            for it in items:
                col_parts.append(f"<div class='px-2 py-1 text-xs md:text-base {bg_cls} rounded mb-1'>{it}</div>")
        
        if not has_content:
            col_parts.append("<div class='text-xs md:text-base text-gray-400'>—</div>")
    ...
    return f"""
    <div class='snap-start md:snap-none shrink-0 w-full md:w-full h-auto md:h-full relative {today_ring} rounded-2xl bg-white border p-4 md:p-8 flex flex-col mb-4 md:mb-0'>
      {today_badge}
      <div class='text-center mb-4 md:mb-6'>
        <div class='text-2xl md:text-4xl font-bold'>{day_name}</div>
        <div class='text-sm md:text-lg text-gray-500'>{dt_title}</div>
      </div>
      
      <div class='flex-1 grid grid-rows-2 gap-4 md:gap-6'>
        <!-- Lunch Row -->
        <div class='border-b pb-2'>
          <div class='text-base md:text-xl font-semibold mb-3'>Lunch</div>
          <div class='grid grid-cols-3 gap-2 md:gap-4 text-xs items-start'>
            <div>{lunch_col1}</div>
            <div>{lunch_col2}</div>
            <div>{lunch_col3}</div>
          </div>
        ...
        <!-- Dinner Row -->
        <div>
          <div class='text-base md:text-xl font-semibold mb-3'>Dinner</div>
          <div class='grid grid-cols-3 gap-2 md:gap-4 text-xs items-start'>
            <div>{dinner_col1}</div>
            <div>{dinner_col2}</div>
            <div>{dinner_col3}</div>
